...
class MyAgent:
    ...
    def planAMove(self) -> bool:
        ...
        if directionX == 0 and directionY == 0:
            if self.isChest():
                ...
                mail = f'assert: open {destination}'
                for agent in self.env.agentSet:
                    self.send(agent, mail)
                    self.env.log += f'{self.id} -> {agent}: {mail}\n'
            elif self.lastPosition == self.env.posUnload:
                self.moves.append(('unload', None, None, None, None))
            else:
                self.moves.append(('load', None, None, None, None))
            self.bigPlan.pop(0)
            return self.planAMove()
        nDirectionX = 0 if directionX == 0 else (1 if directionX > 0 else -1)
        nDirectionY = 0 if directionY == 0 else (1 if directionY > 0 else -1)
        s = [(nDirectionX, 0), (0, nDirectionY)]
        random.shuffle(s)
        for directionX, directionY in s:
            next = self.lastPosition[0] + directionX, self.lastPosition[1] + directionY
            if (directionX != 0 or directionY != 0) and not next in self.previousAgentPosisions.values():
                treasure : Treasure= self.env.grilleTres[next[0]][next[1]]
                if not self.isChest() and next == destination and treasure and not next in self.open_chests:
                    self.moves.append(('wait', None, None, None, None))
                    return True
                self.moves.append(('move', *self.lastPosition, *next))
                self.lastPosition = next
                for agent in self.env.agentSet:
                    mail = f'assert: move {next}'
                    self.send(agent, mail)
                    self.env.log += f'{self.id} -> {agent}: {mail}\n'
                return True
        s = [-1, 1]
    ...
    def handleMail(self) -> bool:
        if self.mailBox:
            ...
            elif mail.startswith('assert: open '):
                treasure = tuple(map(lambda x: int(x), mail[13:].split(', ')))
                self.open_chests.append(treasure)
